...
import { CourseAPI, ICourse } from '@/utils/api/course.api';
...
export async function generateStaticParams() {
  ...
  return courses.result.courses.map((course: ICourse) => ({
    slug: course.slug,
  }))
}
...
export const generateMetadata = async ({
  ...
  const response = await CourseAPI.getOne(params.slug)
  const course = response.result as ICourse;

  const metaData = commonMetaData({
    title: course?.title || 'Not Found | Crystal Pathshala', 
...
const CourseDetails = async ({ params }: { params: { slug: string } }) => {
  ...
  const data = await CourseAPI.getOne(params.slug)

  if (!data.result) {
    return <UnderConstruction title='Course Details' />
  }

  const course = data.result as ICourse;
  const formattedDate = dayjs(course.updatedAt).format('MMMM DD, YYYY, HH:mm a');


  return (
    <div className='min-h-screen bg-gray-50 text-gray-950'>
      
      <article className="px-4 pb-28 pt-10 mx-auto max-w-7xl">
        <div className="w-full mx-auto mb-10 text-left md:w-3/4">
          ...
          <Image src={course.featuredImage} height={500} width={1200} className="object-cover w-full h-full bg-center rounded" alt="crystal-pathshala" />
        </div>

        <div className="w-full mx-auto prose md:w-3/4" dangerouslySetInnerHTML={{ __html: course.description }} />
      </article>
